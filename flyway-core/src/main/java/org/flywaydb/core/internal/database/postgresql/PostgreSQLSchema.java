...
public class PostgreSQLSchema extends Schema<PostgreSQLDatabase, PostgreSQLTable> {
    ...
    private List<String> generateDropStatementsForExtensions() throws SQLException {
        ...
        if (extensionsTableExists()) {
            List<String> extensionNames = jdbcTemplate.queryForStringList("SELECT e.extname " + "FROM pg_extension e " + "LEFT JOIN pg_namespace n ON n.oid = e.extnamespace " + "LEFT JOIN pg_roles r ON r.oid = e.extowner " + "WHERE n.nspname=? AND r.rolname=?", name, database.doGetCurrentUser());
    ...
    private List<String> generateDropStatementsForRoutines() throws SQLException {
        ...
        List<Map<String, String>> rows = jdbcTemplate.queryForList(
                ...
                "SELECT proname, oidvectortypes(proargtypes) AS args, " + isAggregate + " as agg, " + isProcedure + " as proc " + "FROM pg_proc INNER JOIN pg_namespace ns ON (pg_proc.pronamespace = ns.oid) "
                        // that don't depend on an extension
                        + "LEFT JOIN pg_depend dep ON dep.objid = pg_proc.oid AND dep.deptype = 'e' " + "WHERE ns.nspname = ? AND dep.objid IS NULL", name);
    ...
    private List<String> generateDropStatementsForViews() throws SQLException {
        List<String> viewNames = jdbcTemplate.queryForStringList(
                ...
                "SELECT relname FROM pg_catalog.pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace" +
                        // that don't depend on an extension
                        " LEFT JOIN pg_depend dep ON dep.objid = c.oid AND dep.deptype = 'e'" + " WHERE c.relkind = 'v' AND  n.nspname = ? AND dep.objid IS NULL", name);
        List<String> statements = new ArrayList<>();
        for (String domainName : viewNames) {
    ...
    protected PostgreSQLTable[] doAllTables() throws SQLException {
        List<String> tableNames = jdbcTemplate.queryForStringList(
                ...
                "SELECT t.table_name FROM information_schema.tables t" +
                        ...
                        " AND NOT (SELECT EXISTS (SELECT inhrelid FROM pg_catalog.pg_inherits" + " WHERE inhrelid = (quote_ident(t.table_schema)||'.'||quote_ident(t.table_name))::regclass::oid))", name);
