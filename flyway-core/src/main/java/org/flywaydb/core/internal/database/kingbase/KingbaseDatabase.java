...
public class KingbaseDatabase extends Database<KingbaseConnection> {
    private static final Pattern KINGBASE_VERSION_PATTERN =
            Pattern.compile("V(\\d{3})R(\\d{3})", Pattern.CASE_INSENSITIVE);
    /**
    ...
    static boolean isRunningInPerconaXtraDBClusterWithStrictMode(JdbcTemplate jdbcTemplate) {
        try {
            String pcx_strict_mode = jdbcTemplate.queryForString("select VARIABLE_VALUE from performance_schema.global_variables" + " where variable_name = 'pxc_strict_mode'");
    ...
    boolean isMySQL() {
        return databaseType instanceof KingbaseDatabaseType;
    }
    ...
    public String getRawCreateScript(Table table, boolean baseline) {
        ...
        if (baseline) {
            if (isCreateTableAsSelectAllowed()) {
                baselineMarker = " AS SELECT" + "     1 as \"installed_rank\"," + "     '" + configuration.getBaselineVersion() + "' as \"version\"," + "     '" + configuration.getBaselineDescription() + "' as \"description\"," + "     '" + CoreMigrationType.BASELINE + "' as \"type\"," + "     '" + configuration.getBaselineDescription() + "' as \"script\"," + "     NULL as \"checksum\"," + "     '" + getInstalledBy() + "' as \"installed_by\"," + "     CURRENT_TIMESTAMP as \"installed_on\"," + "     0 as \"execution_time\"," + "     TRUE as \"success\"\n";
            } else {
    ...
    static MigrationVersion extractMySQLVersionFromString(String selectVersionOutput) {
        return extractVersionFromString(selectVersionOutput, KINGBASE_VERSION_PATTERN);
    }
    ...
    private static MigrationVersion extractVersionFromString(String versionString, Pattern... patterns) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                int v = Integer.parseInt(matcher.group(1)); // 8
                int r = Integer.parseInt(matcher.group(2)); // 6
                return MigrationVersion.fromVersion(v + "." + r);
            }
        }
    ...
    public final void ensureSupported() {
        ensureDatabaseNotOlderThanOtherwiseRecommendUpgradeToFlywayEdition("8",
                org.flywaydb.core.internal.license.Edition.ENTERPRISE);
        recommendFlywayUpgradeIfNecessary("8");
    }
