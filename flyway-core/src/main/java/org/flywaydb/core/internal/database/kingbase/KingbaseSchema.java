...
public class KingbaseSchema extends Schema<KingbaseDatabase, KingbaseTable> {

    KingbaseSchema(JdbcTemplate jdbcTemplate, KingbaseDatabase database, String name) {
    ...
    protected boolean doEmpty() throws SQLException {
        ...
        return jdbcTemplate.queryForInt("SELECT SUM(found) FROM (" + "(SELECT 1 as found FROM information_schema.tables WHERE table_schema=?) UNION ALL " + "(SELECT 1 as found FROM information_schema.views WHERE table_schema=? LIMIT 1) UNION ALL " + "(SELECT 1 as found FROM information_schema.table_constraints WHERE table_schema=? LIMIT 1) UNION ALL " + "(SELECT 1 as found FROM information_schema.triggers WHERE event_object_schema=?  LIMIT 1) UNION ALL " + "(SELECT 1 as found FROM information_schema.routines WHERE routine_schema=? LIMIT 1)" + (database.eventSchedulerQueryable ? " UNION ALL (SELECT 1 as found FROM information_schema.events WHERE event_schema=? LIMIT 1)" : "") + ") as all_found", params.toArray(new String[0])) == 0;
    }
    ...
    protected KingbaseTable[] doAllTables() throws SQLException {
        List<String> tableNames = jdbcTemplate.queryForStringList("SELECT table_name FROM information_schema.tables WHERE table_schema=?" + " AND table_type IN ('BASE TABLE', 'SYSTEM VERSIONED')", name);

        KingbaseTable[] tables = new KingbaseTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new KingbaseTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new KingbaseTable(jdbcTemplate, database, this, tableName);
    }
}