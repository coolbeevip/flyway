...
@CustomLog
public class KingbaseConnection extends Connection<KingbaseDatabase> {
    private static final String USER_VARIABLES_TABLE_KINGBASE = "performance_schema.user_variables_by_thread";
    ...
    public KingbaseConnection(KingbaseDatabase database, java.sql.Connection connection) {
        super(database, connection);

        userVariablesQuery = "SELECT variable_name FROM " + USER_VARIABLES_TABLE_KINGBASE + " WHERE variable_value IS NOT NULL";
...
//        }
    ...
    private boolean hasUserVariableResetCapability() {
        try {
            ...
            LOG.debug("Disabled user variable reset as " + USER_VARIABLES_TABLE_KINGBASE + "cannot be queried (SQL State: " + e.getSQLState() + ", Error Code: " + e.getErrorCode() + ")");
            return false;
        }
    }
    ...
    protected void doRestoreOriginalState() throws SQLException {
        resetUserVariables();
        jdbcTemplate.execute("SET " + FOREIGN_KEY_CHECKS + "=" + originalForeignKeyChecks);
    }
    ...
    protected Schema doGetCurrentSchema() throws SQLException {
        ...
        // #2206: Kingbase and MariaDB can have URLs where no current schema is set, so we must handle this case explicitly.
        return schemaName == null ? null : getSchema(schemaName);
    }

    @Override
    public Schema getSchema(String name) {
        return new KingbaseSchema(jdbcTemplate, database, name);
    }

//    @Override
//    public <T> T lock(Table table, Callable<T> callable) {
//        if (canUseNamedLockTemplate()) {
//            return new KingbaseNamedLockTemplate(jdbcTemplate, table.toString().hashCode()).execute(callable);
//        }
